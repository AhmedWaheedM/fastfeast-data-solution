...
# ---------------------------
# Script directory (relative paths)
# ---------------------------
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(SCRIPT_DIR, "data")
MASTER_DATA_PATH = os.path.join(DATA_DIR, "master", "metadata.json")

# Script paths
GENERATE_MASTER_SCRIPT = os.path.join(SCRIPT_DIR, "generate_master_data.py")
GENERATE_BATCH_SCRIPT = os.path.join(SCRIPT_DIR, "generate_batch_data.py")
ADD_CUSTOMERS_SCRIPT = os.path.join(SCRIPT_DIR, "add_new_customers.py")
ADD_DRIVERS_SCRIPT = os.path.join(SCRIPT_DIR, "add_new_drivers.py")
GENERATE_STREAM_SCRIPT = os.path.join(SCRIPT_DIR, "generate_stream_data.py")
...
def check_master_data():
    ...
    return os.path.exists(MASTER_DATA_PATH)
...
def main():
    ...
    if not check_master_data():
        ...
        if not run_command(["python", GENERATE_MASTER_SCRIPT], 
    ...
    if not run_command(["python", GENERATE_BATCH_SCRIPT, "--date", run_date],
    ...
    for i, hour in enumerate(hours):
        ...
        if random.random() < 0.6:  # 60% chance before each hour
            ...
            if not run_command(["python", ADD_CUSTOMERS_SCRIPT, "--count", str(new_customers)],
        ...
        if random.random() < 0.4:  # 40% chance for drivers
            ...
            if not run_command(["python", ADD_DRIVERS_SCRIPT, "--count", str(new_drivers)],
        ...
        if not run_command(["python", GENERATE_STREAM_SCRIPT, "--date", run_date, "--hour", str(hour)],
    ...
    print("=" * 60)
    print(f"Total customers added: {stats['customers_added']}")
    print(f"Total drivers added: {stats['drivers_added']}")
    print(f"Total hours simulated: {stats['hours_generated']}")
    print("Orphan opportunities:")
    for o in stats["orphan_opportunities"]:
        print(f"  - {o}")
